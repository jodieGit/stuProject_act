...
const halfBallSize = userDefined.ballSize/2;
var mouse = {};
var oldMouse = {};
var balls = [];
...
// *******************
// function to casually GC
// look again at this later!
// *******************
var cleaning = false;
...
(function loop() {
  ...
  for (let i = 0, length = balls.length; i < length; i++) {
    ...
    if (balls[i].x < 0) {
      balls[i].x = 0;      
    } else if (balls[i].x > windowDimensions.width) {
      balls[i].x = windowDimensions.width;      
    }
    
    // bounce off
    if (balls[i].x <= 0 || balls[i].x >= windowDimensions.width) {
